...
@Slf4j
// @Component: Registra la clase en Spring para poder inyectarla en el servicio
// de vehículos.
@Component
public class StateTransitionValidator {
    // PATRÓN DE DISEÑO APLICADO: Finite State Machine (FSM - Máquina de Estados
    // Finitos).
    // Define una "Aduana" estricta de hacia dónde puede moverse un vehículo. Evita
    // saltos ilógicos.
    // Ejemplo: Un avión no puede pasar de estar "Volando" a "Estacionado" sin pasar
    // por "Aterrizando".

    // Creamos un diccionario gigante y altamente optimizado (EnumMap) que
    // funcionará como nuestro "Mapa de Rutas Permitidas".
    private static final Map<EstadoVehiculo, Set<EstadoVehiculo>> TRANSICIONES_PERMITIDAS = new EnumMap<>(
            EstadoVehiculo.class);

    // Bloque 'static': Se ejecuta UNA SOLA VEZ cuando arranca el programa. Llena el
    // mapa con las reglas oficiales.
    static {
        // REGLAS DE NEGOCIO: Rutas de estado.
        // Si el camión está DISPONIBLE, solo se le permite saltar a RESERVADO,
        // MANTENIMIENTO o FUERA_DE_SERVICIO.
        TRANSICIONES_PERMITIDAS.put(EstadoVehiculo.DISPONIBLE, EnumSet.of(EstadoVehiculo.RESERVADO,
                EstadoVehiculo.EN_MANTENIMIENTO, EstadoVehiculo.FUERA_DE_SERVICIO));

        // Si el camión está RESERVADO, puede volver a DISPONIBLE o dañarse
        // (FUERA_DE_SERVICIO), pero NO puede saltar directo al taller.
        TRANSICIONES_PERMITIDAS.put(EstadoVehiculo.RESERVADO, EnumSet.of(EstadoVehiculo.DISPONIBLE,
                EstadoVehiculo.EN_MANTENIMIENTO, EstadoVehiculo.FUERA_DE_SERVICIO));

        // Si el camión está EN MANTENIMIENTO, solo puede ser liberado (DISPONIBLE) o
        // declarado inservible (FUERA_DE_SERVICIO).
        ...
        // Si el camión está FUERA DE SERVICIO, la ÚNICA salida es pasar por DISPONIBLE
        // tras ser revisado.
    ...
    public boolean isValidTransition(EstadoVehiculo estadoActual, EstadoVehiculo estadoNuevo) {
        ...
        // Si el estado no cambia (se queda igual), se considera un movimiento válido
        // (no hace daño).
        if (estadoActual == estadoNuevo) {
            return true;
        }

        // Busca en el diccionario cuáles son las rutas permitidas partiendo desde el
        // "estadoActual".
        ...
        // Ejemplo: Si intentas pasar de MANTENIMIENTO a RESERVADO directo, el sistema
        // ve que no está en la lista y retorna falso.
    ...
    public void validateTransition(EstadoVehiculo estadoActual, EstadoVehiculo estadoNuevo) {
        ...
        if (!isValidTransition(estadoActual, estadoNuevo)) {
            ...
            // Lanza una excepción que bloquea la base de datos y le avisa al cliente que su
            // petición es ilegal.
            // Ejemplo: Le muestra al mecánico en pantalla: "Transición de estado no
            // permitida: EN_MANTENIMIENTO → RESERVADO".
            throw new IllegalStateException(
                    "Transición de estado no permitida: " + estadoActual + " → " + estadoNuevo);
        }

        // Si el cambio era legal, deja una nota oculta (modo debug) para los
        // desarrolladores de que todo salió bien.
